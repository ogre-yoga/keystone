...
![Admin UI showing create screen for a blog post](/assets/walkthroughs/lesson-2/my-first-post.png)
...
const lists = {
  ...
  Post: list({
    fields: {
      ...
      // We’re using author as a more user friendly field name
    },
  }),
};
...
![Admin UI showing creation of a nerw post, and relationship to a user from the post ](/assets/walkthroughs/lesson-2/my-first-relationship.gif)
...
![Admin UI showing create screen for a blog post](/assets/walkthroughs/lesson-2/relationship-edit-cards-display.gif)
...
Next up, we’ll add publishing workflows to our app using the `select` and `timestamp` fields.
